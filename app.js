...
io.on('connection', function(socket) {
    ...
    socket.on('newUser', function(data, cb) {
        ...
        if (findUser(data.name,Lobby.users)) {
    ...
    socket.on('newRoom', function(data, cb) {
        if (findRoom(data.room, rooms)) {
            ...
            cb({
                bool: false,
                message: "That room already exists.  Either join it or create a new one!"
            })
        } else if (socket.currentRoom) {

            cb({
                bool: false,
                message: "You've already joined a room!"
            })
            ...
            io.to(socket.id).emit('currentRoom', {
                currentRoom: room
            })
            cb({
                    bool: true,
    ...
    socket.on('disconnect', function(data) {
        if (socket.user) {
            ...
            if (socket.currentRoom) {
                ...
                io.to(socket.currentRoom.name).emit('updatedCurrentRoom', {
                    currentRoom: room
                })
            }
            if(socket.currentGame){
                socket.currentGame.removeUser(socket.user)
                io.to(socket.currentRoom.name).emit('game',socket.currentGame)
            }
        }
    })
    ...
    socket.on('joinRoom', function(data, cb) {
        if (socket.currentRoom) {
            cb({
                bool: false,
                message: 'You Have already Joined a room'
            })
        } else {
            ...
            io.to(data.room.name).emit('updatedCurrentRoom', {
                currentRoom: room
            })
    ...
    socket.on('startGame', function(game) {
        ...
        var game = new Game(usersArray, newGameOptions.Lady, rolesArray, room.name, newGameOptions.rounds,newGameOptions.cards);
        ...
        game.host=socket.currentRoom.host
        if (game.Lady.inUse) {
    ...
    socket.on('ladySubmit', function(data, cb) {
        console.log("Made it to the server emit")
        if (false) {
            cb({
                bool: false
            })
        } else {
            var game = ServerGames[socket.currentRoom.name];
            var user = game.rolesArray.find(function(user) {
                return user.id === data.id
            })
            io.to(socket.id).emit('ladyInfo', {
                name: user.name,
                allegiance: user.allegiance
            })
            game.game.nextLady(user.name);
        }
    })

    socket.on('nextState', function(data) {
        var game = ServerGames[socket.currentRoom.name].game;
        game.nextState();
        io.emit('game', game)
    })

    socket.on('testGame', function(data) {
        var namesArray = ["Ted", "Gretchen", "Bill", "Ruby", "Mike", "Fran", "Brian", "Sarah"];
        var socketNum = 0;
        namesArray.forEach(function(name) {
            var user = new User(socketNum, name);
            socketNum++;
            Lobby.addUser(user);
            var room = socket.currentRoom;
            console.log('ROOM: ', room)
    ...
    socket.on('teamSubmit', function(data) {
        console.log(Object.keys(socket))
        var game = ServerGames[socket.currentRoom.name].game;
        game.nextState()
        game.suggestedTeam = data;
        io.emit('game', game)
        io.emit('teamVote', {
    ...
    socket.on('vote', function(data) {
        ...
        if (game.addVote('team', vote)) {
            ...
            game.reset();
            if (outcome === 'Passes') {
                game.roundOfVoting = 1;
                game.nextState('Quest');
            } else if (outcome === 'Fails') {
                console.log("Failed")
                game.nextLeader();
                game.roundOfVoting++;
                console.log(game.leader)
                game.nextState('Selection');
                console.log(ServerGames[socket.currentRoom.name].game)
            }
            setTimeout(function() {
                io.emit('game', game)
    ...
    socket.on('questVote', function(data) {
        ...
        if (game.addVote('quest', data)) {
            var outcome = game.checkQuest();
            io.emit('questVotingDone', {
                votes: game.votes,
                outcome: outcome
            })
            game.reset();
...
var findUser = function(username, array){
    console.log(array)
    var found = array.filter(function(user){
